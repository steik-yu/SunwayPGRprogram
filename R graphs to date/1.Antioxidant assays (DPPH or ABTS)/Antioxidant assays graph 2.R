...
DPPHABTS$Concentration <- DPPHABTS$Not_same_unit_concentration
...
# Calculate scaling factor to normalize ascorbic acid concentrations
max_sample_conc <- max(NormDPPHABTS %>% 
                         filter(Group == "DPPH", Treatment != "Ascorbic") %>% 
                         pull(Concentration))

max_asc_conc <- max(NormDPPHABTS %>% 
                      filter(Group == "DPPH", Treatment == "Ascorbic") %>% 
                      pull(Concentration))

scaling_factor <- max_sample_conc / max_asc_conc

# Prepare data with normalized concentrations
plot_data_DPPH <- NormDPPHABTS %>%
  filter(Group == "DPPH") %>%
  mutate(
    # Scale ascorbic acid concentrations to 0-10 range
    Normalized_Conc = ifelse(Treatment == "Ascorbic",
                             Concentration * scaling_factor,
                             Concentration)
  ) 
...
plot1 <- ggplot(plot_data_DPPH, aes(x = Normalized_Conc, y = Mean_ScavengingActivity_P, 
                                 color = Treatment)) +
  geom_line(aes(color = Treatment), size = 1, alpha = 0.5) +
  geom_point(aes(shape = Treatment, color = Treatment)) +
  theme_bw() +
  labs(
    x = "Concentration for Treatment (g/mL)",
    y = "DPPH % of RSA"
  ) +
  scale_x_continuous(
    sec.axis = sec_axis(
...
# Calculate scaling factor to normalize ascorbic acid concentrations
max_sample_conc <- max(NormDPPHABTS %>% 
                         filter(Group == "ABTS", Treatment != "Ascorbic") %>% 
                         pull(Concentration))
...
plot2 <- ggplot(plot_data_ABTS, aes(x = Normalized_Conc, y = Mean_ScavengingActivity_P, 
                                    color = Treatment)) +
  geom_line(aes(color = Treatment), size = 1, alpha = 0.5) +
  geom_point(aes(shape = Treatment, color = Treatment)) +
  theme_bw() +
